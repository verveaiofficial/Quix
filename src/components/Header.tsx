...
export const Header: React.FC<HeaderProps> = ({
  currentMode: externalMode = 'Quix 3 Flash',
  chatTitle: externalTitle = 'New Chat',
  onModeChange,
  onToggleSidebar,
  onRenameChat,
  onDeleteChat,
}) => {
  const [currentMode, setCurrentMode] = useState<string>(externalMode);
  const [chatTitle, setChatTitle] = useState<string>(externalTitle);
  ...
  const handleSetMode = (mode: string, e: React.MouseEvent) => {
    e.stopPropagation();
    setCurrentMode(mode);
    setIsModeMenuOpen(false);
  ...
  const handleCloseRename = () => setIsRenameModalOpen(false);

  const handleSaveRename = () => {
    const trimmed = renameInputVal.trim();
    if (trimmed) {
      setChatTitle(trimmed);
      if (onRenameChat) onRenameChat(trimmed);
    }
  ...
  return (
    <>
      <style>{`
        ...
        *:focus, *:active, *::focus-visible { outline: none !important; box-shadow: none !important; -webkit-tap-highlight-color: transparent !important; }
        ...
        #chat-header {
            ...
            background: rgba(18,18,22,0.94); border-bottom: 1px solid rgba(255,255,255,0.12);
        ...
        .mode-menu {
            position: fixed; top: 64px; left: 50%; transform: translate(-50%, -12px);
            background: rgba(18,18,22,0.96); border: 1px solid rgba(255,255,255,0.12); border-radius: 16px;
            width: 260px; padding: 6px 4px; display: flex; flex-direction: column; gap: 4px;
            backdrop-filter: blur(40px) saturate(200%) brightness(1.1);
            -webkit-backdrop-filter: blur(40px) saturate(200%) brightness(1.1);
            box-shadow: 0 16px 40px rgba(0,0,0,0.6), 0 1px 0 rgba(255,255,255,0.08) inset;
            z-index: 25; opacity: 0; visibility: hidden; pointer-events: none;
            transition: opacity 0.28s cubic-bezier(0.4, 0, 0.2, 1), transform 0.28s cubic-bezier(0.4, 0, 0.2, 1), visibility 0.28s cubic-bezier(0.4, 0, 0.2, 1);
            overflow: hidden;
            font-family: 'Inter', -apple-system, sans-serif;
        }
        ...
        #chat-options-menu {
            position: fixed; top: 64px; right: 14px; background: rgba(18,18,22,0.96);
            border: 1px solid rgba(255,255,255,0.12); border-radius: 16px; overflow: hidden; min-width: 180px;
            backdrop-filter: blur(40px) saturate(200%) brightness(1.1);
            -webkit-backdrop-filter: blur(40px) saturate(200%) brightness(1.1);
            box-shadow: 0 16px 40px rgba(0,0,0,0.6), 0 1px 0 rgba(255,255,255,0.08) inset;
            z-index: 25; opacity: 0; pointer-events: none;
            transition: opacity 0.18s cubic-bezier(0.25, 0.46, 0.45, 0.94), visibility 0.18s cubic-bezier(0.25, 0.46, 0.45, 0.94);
            font-family: 'Inter', -apple-system, sans-serif;
        }
        ...
        #rename-modal { position:fixed; inset:0; z-index:200; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,.7); backdrop-filter:blur(6px); opacity:0; pointer-events:none; transition:opacity .2s; font-family: 'Inter', -apple-system, sans-serif; }
      ...
      <div id="chat-header">
        <button className="chat-hamburger" onClick={onToggleSidebar} aria-label="Toggle Menu">
          <span></span><span></span><span></span>
        </button>

        <div className="header-center">
          <div className={`status-badge ${isModeMenuOpen ? 'active' : ''}`} onClick={handleModeMenuToggle}>
            <span style={{ display: 'inline-flex', alignItems: 'center' }}>
              {currentMode}
              {currentMode === 'DeepThink' && <span className="beta-tag">Beta</span>}
            </span>
        ...
        <button className={`hdr-dots-btn ${isOptionsMenuOpen ? 'active' : ''}`} onClick={handleOptionsMenuToggle} aria-label="Chat Options">
      ...
      <div className={`mode-menu ${isModeMenuOpen ? 'show' : ''}`}>
        <div className="mode-item" onClick={(e) => handleSetMode('Quix 3 Flash', e)}>
          {currentMode === 'Quix 3 Flash' ? (
            <svg viewBox="0 0 24 24" fill="none" className="mode-checkmark-svg" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
          ) : (<div style={{ width: '15px', flexShrink: 0 }}></div>)}
          <div className="mode-item-content">
            <span className="mode-title">Quix 3 Flash</span>
            <span className="mode-desc">Balanced intelligence for daily tasks</span>
          </div>
        </div>
        <div className="mode-item" onClick={(e) => handleSetMode('Quix 3 Lite', e)}>
          {currentMode === 'Quix 3 Lite' ? (
            <svg viewBox="0 0 24 24" fill="none" className="mode-checkmark-svg" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
          ) : (<div style={{ width: '15px', flexShrink: 0 }}></div>)}
          <div className="mode-item-content">
            <span className="mode-title">Quix 3 Lite</span>
            <span className="mode-desc">Instant replies</span>
          </div>
        </div>
        <div className="mode-item" onClick={(e) => handleSetMode('Quix 3 Coder', e)}>
          {currentMode === 'Quix 3 Coder' ? (
            <svg viewBox="0 0 24 24" fill="none" className="mode-checkmark-svg" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
          ) : (<div style={{ width: '15px', flexShrink: 0 }}></div>)}
          <div className="mode-item-content">
            <span className="mode-title">Quix 3 Coder</span>
            <span className="mode-desc">Build apps and sites</span>
          </div>
        </div>
        <div className="mode-item" onClick={(e) => handleSetMode('Imagine 1.5', e)}>
          {currentMode === 'Imagine 1.5' ? (
            <svg viewBox="0 0 24 24" fill="none" className="mode-checkmark-svg" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
          ) : (<div style={{ width: '15px', flexShrink: 0 }}></div>)}
          <div className="mode-item-content">
            <span className="mode-title">Imagine 1.5</span>
            <span className="mode-desc">Generate unique images with Quix</span>
          </div>
        </div>
        <div className="mode-item" onClick={(e) => handleSetMode('DeepThink', e)}>
          {currentMode === 'DeepThink' ? (
            <svg viewBox="0 0 24 24" fill="none" className="mode-checkmark-svg" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
          ) : (<div style={{ width: '15px', flexShrink: 0 }}></div>)}
          <div className="mode-item-content">
            <span className="mode-title">DeepThink<span className="beta-tag">Beta</span></span>
            <span className="mode-desc">5-minute deep research & reasoning</span>
          </div>
        </div>
      </div>

      <div id="chat-options-menu" className={isOptionsMenuOpen ? 'show' : ''}>
      ...
      <div id="rename-modal" className={isRenameModalOpen ? 'show' : ''} onClick={handleCloseRename}>
        <div className="rename-box" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="rename-actions">
            <button className="rename-cancel" onClick={handleCloseRename}>Cancel</button>
            <button className="rename-save" onClick={handleSaveRename}>Save</button>
          </div>
        </div>
      </div>
    </>
