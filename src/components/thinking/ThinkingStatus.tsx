...
import { SourceItem } from "../../store/chatStore";
...
.qts-toggle { display: none; align-items: center; justify-content: center; width: 12px; height: 12px; color: #8a8f96; cursor: pointer; background: none; border: none; padding: 0; transition: transform .3s ease; }
.qts-status.done .qts-toggle { display: inline-flex; }
.qts-toggle.open { transform: rotate(180deg); }
.qts-toggle svg { stroke: currentColor; }
.qts-sep { color: #55585e; font-size: 12px; flex-shrink: 0; line-height: 1.2; }
.qts-sources-btn { background: transparent; border: none; outline: none; font-family: inherit; font-size: 12.5px; color: #8a8f96; padding: 2px 6px 2px 2px; border-radius: 6px; cursor: pointer; display: inline-flex; align-items: center; gap: 4px; white-space: nowrap; line-height: 1.2; transition: background-color .3s ease, color .3s ease; }
.qts-sources-btn:hover { background-color: rgba(255,255,255,0.06); color: #f5f5f5; }
.qts-sources-btn .qts-chev { display: inline-flex; transition: transform .3s ease; }
.qts-sources-btn.open .qts-chev { transform: rotate(180deg); }
.qts-chev svg { stroke: currentColor; }
.qts-sources-list { max-height: 0; overflow: hidden; margin-left: 6px; display: flex; flex-direction: column; gap: 6px; opacity: 0; transition: max-height .4s ease, margin-top .3s ease, opacity .3s ease, margin-bottom .3s ease; }
.qts-sources-list.open { max-height: 180px; margin-top: 6px; margin-bottom: 8px; opacity: 1; }
.qts-source-item { font-size: 12px; color: #8a8f96; text-decoration: none; padding: 5px 9px; border-radius: 6px; background: rgba(255,255,255,0.03); transition: background-color .2s ease, color .2s ease; width: fit-content; display: inline-flex; align-items: center; gap: 6px; }
.qts-source-item:hover { background: rgba(255,255,255,0.08); color: #f5f5f5; }
.qts-source-item svg { stroke: currentColor; }
.qts-steps { padding: 2px 0 6px 0; margin-left: 6px; display: flex; flex-direction: column; overflow: hidden; transition: max-height .25s cubic-bezier(0.4, 0, 0.2, 1), opacity .2s ease; }
.qts-steps.hidden { max-height: 0; opacity: 0; padding: 0 0 0 6px; }
.qts-step { position: relative; display: flex; align-items: center; gap: 10px; padding: 6px 0 6px 26px; font-size: 13px; color: #6b6f76; opacity: 0; transform: translateY(4px); transition: opacity .35s ease, transform .35s ease; }
.qts-step.show { opacity: 1; transform: translateY(0); }
.qts-step::before { content: ""; position: absolute; left: 6px; top: calc(-50% + 9px); bottom: calc(50% + 9px); width: 1px; background: #26262a; }
.qts-step:first-child::before { display: none; }
.qts-step-icon { position: absolute; left: 0; top: 50%; transform: translateY(-50%); width: 12px; height: 12px; display: flex; align-items: center; justify-content: center; color: #3f4247; }
.qts-step-icon svg { display: block; stroke: currentColor; }
.qts-step-text { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
`;
    ...
    <div className={`qts-status visible ${done ? "done" : "active"}`}>
      ...
      <div className="qts-head">
        ...
        <div className="qts-title-row">
          ...
          {hasSources && <span className="qts-sep">•</span>}

          {hasSources && (
            <button
              className={`qts-sources-btn ${sourcesOpen ? "open" : ""}`}
              onClick={(e) => {
                e.stopPropagation();
      ...
      {hasSources && (
        <div className={`qts-sources-list ${sourcesOpen ? "open" : ""}`}>
          {sources!.map((source, index) => (
            <a
              key={index}
              className="qts-source-item"
              href={source.uri}
              target="_blank"
              rel="noreferrer"
            >
